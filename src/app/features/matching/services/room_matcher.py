...
from app.features.matching.schemas.schemas import RoomMatchRequest, RoomMatchResult
    ...
    def match_rooms(self, current_account_id: int, request: RoomMatchRequest) -> List[RoomMatchResult]:
        ...
        if not current_pref:
            raise HTTPException(
                ...
                detail="Please create a matching profile before matching rooms."
            )
        ...
        for room in candidate_rooms:
            ...
            if request.require_wifi or request.require_parking:
                ...
                if request.require_parking and (
                    "giu xe" in desc or "parking" in desc or "de xe" in desc
                ):
                    score += 5
